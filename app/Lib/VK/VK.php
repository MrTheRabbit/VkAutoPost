...
class VK
{
    ...
    public function setModeEnterCapcha($booMode) {
    	$this->booEnterCapcha = ($booMode == true);
    }
    ...
    public function api($method, $parameters = array(), $format = 'array', $requestMethod = 'get')
    {
        ...
        if ($this->booEnterCapcha) {
        	$arrR = json_decode($rs, true);

			if (isset($arrR['error']['error_code']) && intval($arrR['error']['error_code']) == 14) {
				echo "\n".'Введите капчу ['.$arrR['error']['captcha_img'].']:';
				$strCaptchaKey  = fgets(STDIN, 255);
				
				$arrAddParameters = [
					'captcha_sid' => $arrR['error']['captcha_sid'],
					'captcha_key' => $strCaptchaKey,
				];
				$arrSaveParameters = array_merge($arrSaveParameters, $arrAddParameters);
				return $this->api($method, $arrSaveParameters, $format, $requestMethod);
			}//\\ if 
        }//\\ if
